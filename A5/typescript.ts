namespace Save {
    ...
    class SaveSave {
        ...
        constructor(interpret: string, preis: string) {
    ...
    if (localStorage.length > 0) {
        let interpretValue: string;
        let preisValue: number;

        let localIndex: number = 0;
        //Wieviele Elemente sind drin?
        for (let i: number = 0; i < localStorage.length; i++) {

            
            while(localStorage.getItem(localIndex.toString()) === null){
                localIndex++;
            }
            

            interpretValue = JSON.parse(localStorage.getItem(localIndex.toString())).interpret;
            preisValue = parseInt(JSON.parse(localStorage.getItem(localIndex.toString())).preis);
            
            localStorage.removeItem(localIndex.toString());
            localIndex++;
            addEventElement(interpretValue, preisValue);
    ...
    function addEventElement(interpretValue: string, preisValue: number) {
        ...
        newElement.id = index.toString();


        localStorage.setItem(index.toString(), JSON.stringify(new SaveSave(interpretValue, preisValue.toString())));
    ...
    function deleteEvent(parentElement: HTMLDivElement): void {
        ...
        localStorage.removeItem(parentElement.id.toString());
    }
...
let eventtabellen: Eventtabellen = new Eventtabellen();
